...
export default function AboutPage() {
  ...
  const domainChipRefs = useRef<(HTMLButtonElement | null)[]>([]);
  ...
  // ─── Scroll active domain chip into view ───
  const hasMountedRef = useRef(false);
  useEffect(() => {
    // Skip on initial mount so the row starts at scrollLeft: 0
    // and the first pill isn't clipped by a scrollIntoView call.
    if (!hasMountedRef.current) {
      hasMountedRef.current = true;
      return;
    }
    const chip = domainChipRefs.current[domainIdx];
    if (chip) {
      chip.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
    }
  }, [domainIdx, committeeIdx]);
      ...
      <main>
        ...
        <section className="about-hero" id="about-hero">
          ...
          <div className="about-hero__content">
            ...
            <h1 className="ab-heading-xl about-hero__title" data-reveal style={{ "--reveal-delay": "0.15s" } as React.CSSProperties}>
              CODESTORM
            </h1>
            ...
            <nav className={`domains${committee.domains.length <= 1 ? " is-hidden" : ""}`} aria-label="Committee domains">
              {committee.domains.length > 1 && committee.domains.map((d, i) => (
                <button
                  ...
                  ref={el => { domainChipRefs.current[i] = el; }}
                  onClick={() => {
