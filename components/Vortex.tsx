...
function createVortex(
  ...
  function waveAt(
    ...
    now: number, out: Float32Array, at: number, radius: number
  ) {
    let ox = 0, oy = 0, oz = 0;
    let any = false;
    for (let i = 0; i < MAX_WAVES; i++) {
    ...
    out[at] += ox; out[at + 1] += oy; out[at + 2] += oz;
  }
  ...
  function burstAt(i: number, now: number, force: number) {
    ...
    const maxSq = RIPPLE_RADIUS * RIPPLE_RADIUS;
    for (let j = 0; j < dotCount; j++) {
      const dx = dotHome[j * 3] - x, dy = dotHome[j * 3 + 1] - y, dz = dotHome[j * 3 + 2] - z;
      const sq = dx * dx + dy * dy + dz * dz;
      if (sq > maxSq || sq < 1e-4) continue;
      const d = Math.sqrt(sq);
      const f = 1 - d / RIPPLE_RADIUS;
  ...
  function drawStrand(strand: Strand, now: number, entrance: number) {
    ...
    for (let i = 0; i < STRAND_SEGMENTS; i++) {
      ...
      let edge = 1;
      if (u < FADE_ZONE) {
        const k = u / FADE_ZONE; edge = k * k;
      } else if (u > 1 - FADE_ZONE) {
        const k = (1 - u) / FADE_ZONE; edge = k * k;
      }
      let tip = 1;
      if (s > reach) tip = 0;
      else if (s > reach - tipFade) {
        tip = (reach - s) / tipFade; tip *= tip;
      }
  ...
  function driveComet(comet: Comet, now: number, dt: number, entrance: number) {
    ...
    if (comet.boost > 0) {
      comet.boost -= dt;
      if (comet.boost <= 0) {
        comet.boost = 0; comet.boostMul = 1;
      } else {
  ...
  function collide(now: number) {
    ...
    for (const comet of cometList) {
      ...
      for (let i = 0; i < dotCount; i += 3) {
        const dx = dotHome[i * 3] - x, dy = dotHome[i * 3 + 1] - y, dz = dotHome[i * 3 + 2] - z;
        const sq = dx * dx + dy * dy + dz * dz;
  ...
  function step(now: number) {
    ...
    if (!cfg.running) {
      if (!heldFrame) { renderer.render(scene, camera); heldFrame = true; }
      return;
    }
    heldFrame = false;
      ...
      for (let i = 0; i < dotCount; i++) {
        ...
        if (dotHitAt[i] > 0) {
          ...
          if (age < HIT_FADE) {
            ...
            dotAlive[i] = 0; dotFlash[i] = 0;
          }
          if (age > HIT_RESPAWN) { dotHitAt[i] = 0; dotAlive[i] = 1; dotFlash[i] = 0; }
        }
